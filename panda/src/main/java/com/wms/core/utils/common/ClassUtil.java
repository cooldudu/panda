...
public class ClassUtil {
	...
	private static String[] getPackageAllClassName(String classLocation,
												   String packageName) {
		// 将packageName分解
		var packagePathSplit = packageName.split("[.]");
		var realClassLocation = new StringBuffer(classLocation);
		var packageLength = packagePathSplit.length;
		for (var i = 0; i < packageLength; i++) {
			realClassLocation.append(realClassLocation).append(File.separator).append(packagePathSplit[i]);
		}
		var packeageDir = new File(realClassLocation.toString());
		if (packeageDir.isDirectory()) {
			var allClassName = packeageDir.list();
			return allClassName;
		}
		return null;
	}
		...
		try {
			...
			while (dirs.hasMoreElements()) {
				...
				if ("file".equals(protocol)) {
					// 获取包的物理路径
					var filePath = URLDecoder.decode(url.getFile(), "UTF-8");
	...
	private static void findAndAddClassesInPackageByFile(String packageName,
		...
		var dirfiles = dir.listFiles(new FileFilter() {
	...
	public static Set<Class<?>> getPackageClassesExceptMyself(Class clazz) {
		var result = getClasses(clazz.getPackage());
		result.remove(clazz);
		return result;
	}
		...
		try {
			...
			for (var method : methods) {
				...
				if (StringUtils.hasText(fieldName)) {
					var type = method.getParameterTypes()[0].getSimpleName();
				...
				if (null != fieldName
					...
					for (var field : fields) {
						if (field.indexOf(fieldName + ".") != -1
							...
							var fn = new StringBuffer();
							for (var fieldTmp : fieldStrings) {
								var tmpFieldName = fieldTmp.substring(0, 1)
										.toUpperCase()
										+ fieldTmp.substring(1,
										fieldTmp.length());
								var tmpMethod = obj.getClass().getMethod(
										"get" + tmpFieldName);
